...
| 2026-05-09 | **Rolling-origin CV (3 chronologically-valid folds, n=135) — n=83 held-out was optimistic. True forward 1X2 Brier=0.2156, 95% CI [0.2027, 0.2290] straddles the 0.21 gate.** | Random K-fold breaks walk-forward causality (a 2024 match can't train a 2018 prediction). Rolling-origin (Hyndman & Athanasopoulos 2018 §5.10) trains each fold on tournaments that finished BEFORE the held-out — the only valid CV for autoregressive predictors. Default folds: F1=AFCON2023 (179 train), F2=Copa2024 (231 train), F3=Euro2024 (263 train). Aggregate verdict on the SOTA xG-blended predictor (ρ=0.0, α=0.20): 1X2 Brier cal=0.2156 (vs 0.2083 from n=83 — single-held-out sample bias of +0.007), 1X2 ECE cal=0.1074 (calibrator overfits at n<500 — Walsh & Joshi 2024 warning realised). 95% CI on 1X2 Brier straddles the 0.21 ceiling — P(true ≤ 0.21) ≈ 30%; predictor is honestly below-gate in mean. Per-fold breakdown reveals AFCON 2023 is the worst regime (1X2 Brier=0.2271 cal); single-tournament held-outs would never have surfaced this. BTTS Brier 0.2542, OU2.5 Brier 0.2536 — both clearly above gate at n=135. Tests: 823 passing (was 815; +8 Layer-1) |
