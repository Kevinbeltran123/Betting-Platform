...
**Goal**: The full pipeline runs autonomously under operator supervision, with scheduled jobs, graceful degradation when external services fail, health monitoring, and CLV-based performance alerting. Deployment target is host-agnostic — see PROJECT.md "Infrastructure Investment Gate" for when local-mode (default) graduates to a paid VPS.
**Depends on**: Phase 3
...
**Success Criteria** (what must be TRUE):
  ...
  3. The `bip.production` process recovers from crashes via a host-appropriate supervision mechanism, AND a heartbeat at `Settings.heartbeat_file_path` is updated every 5 minutes for liveness verification. Two supervision modes are supported: (a) **Local mode (default, pre-ROI)** — operator runs `python -m bip.production` in a tmux/screen session per `deploy/local/RUNBOOK.md`; recovery is manual. `sdnotify` calls no-op silently. (b) **VPS mode (post-gate)** — systemd unit with `Type=notify + WatchdogSec=600 + Restart=always` per `deploy/systemd/bip.service`. Mode (b) is gated by the Infrastructure Investment Gate in PROJECT.md, NOT a phase exit criterion.
